...
const nav = [
  ...
  { href: "/regulars", label: "Regulars club" },
];
    ...
    <header className="sticky top-0 z-40 border-b border-ink/10 bg-cream/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <Link href="/" className="group flex flex-col leading-none">
          ...
          <span className="text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-ink-soft">
            Coffee · Bakery · Bar
          </span>
        </Link>
      ...
      >
        {nav.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="whitespace-nowrap text-xs font-semibold uppercase tracking-wide text-ink-soft"
          >
            {item.label}
          </Link>
        ))}
